...
switch ($action) {
	...
	function doInsert(){
		...
		if ( $_POST['FNAME'] == "" OR $_POST['LNAME'] == ""
			...
			if ($age < 18){
			message("Invalid age. Employer should be 18 years old.", "error");
			redirect("index.php?view=add");
				...
				$sql = "SELECT * FROM tblemployer WHERE EMPLOYEEID='" .$_POST['EMPLOYEEID']. "'";
				$mydb->setQuery($sql);
				...
				if ($maxrow > 0) { 
					# code... 
					message("Employer ID already in use!", "error");
					redirect("index.php?view=add");
				}else{
					...
					message("New employer added successfully!", "success");
					redirect("index.php");
	...
	if(isset($_POST['save'])){
		...
		if ( $_POST['FNAME'] == "" OR $_POST['LNAME'] == ""
			...
			$age = date_diff(date_create($birthdate),date_create('today'))->y;
		 	if ($age < 18 ){
		       message("Invalid age. Employer should  be above 18 years old.", "error");
		       redirect("index.php?view=edit&id=".$_POST['EMPLOYEEID']);
				...
				message("Employer has been updated!", "success");
	...
	function doDelete(){
		...
		// }
			message("Employer Deleted!","success");
			redirect('index.php');
		// }
